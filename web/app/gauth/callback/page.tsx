  ...
  return (
    <section className="container grid min-h-[68vh] place-items-center py-16">
      <div className="card-shadow w-full max-w-xl rounded-[2rem] border border-[#dbe8eb] bg-white p-8 text-center sm:p-12">
        ...
        <p className="mt-7 text-sm font-bold tracking-[0.12em] text-[#079cb8]">GOOGLE SIGN-IN</p>
        <h1 className="mt-3 text-3xl font-bold tracking-[-0.04em]">
          {isError ? copy.title : state === 'connecting' ? 'Finishing your sign-in' : 'You’re signed in to Card Nest'}
        </h1>
        <p aria-live="polite" className="mt-5 leading-7 text-[#60767c]">
          {isError
            ? copy.body
            : state === 'connecting'
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          {state === 'success' ? (
            <a
              className="focus-ring inline-flex min-h-12 items-center justify-center rounded-xl bg-[#079cb8] px-6 font-bold text-white hover:bg-[#067a90]"
              href={appLink}>
              Open Card Nest
            </a>
          ) : null}
          <Link
            className="focus-ring inline-flex min-h-12 items-center justify-center rounded-xl border border-[#bfd5da] px-6 font-bold text-[#334a50] hover:border-[#0CC0DF]"
            href="/">
            Return home
          </Link>
        </div>
        {state === 'success' ? (
          <p className="mt-6 text-sm leading-6 text-[#8aa0a6]">
