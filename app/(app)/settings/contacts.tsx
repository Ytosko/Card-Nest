...
import { isSavedContact, markCardExported } from '@/src/features/cards/card-service';
...
export default function ContactsSettingsScreen() {
  ...
  // Only saved contacts — failed-extraction placeholders are not exportable.
  const exportableCards = (cards.data ?? []).filter(isSavedContact);
  function toggle(id: string) { setSelected((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; }); }
  async function exportSelected() { const chosen = exportableCards.filter((card) => selected.has(card.id)); if (!chosen.length) return; setBusy(true); setError(null); setNotice(null); try { const result = await exportCardsToContacts(chosen); await Promise.all(result.succeeded.map(({ cardId }) => markCardExported(cardId))); const completed = result.succeeded.length; if (completed) setNotice(`${completed} ${completed === 1 ? 'contact' : 'contacts'} saved to your device.`); if (result.failed.length) setError(`${result.failed.length} ${result.failed.length === 1 ? 'contact was' : 'contacts were'} not saved. ${result.failed[0].message}`); setSelected(new Set(result.failed.map(({ cardId }) => cardId))); await cards.refetch(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Export stopped before completion.'); } finally { setBusy(false); } }
