...
export function logUpdateDiagnostic(event: string, meta: Record<string, any> = {}) {
  const payload = {
    ...
    runtimeVersion: Updates.runtimeVersion || '0.1.5-alpha-e4f81',
...
export function getCurrentVersionInfo(): VersionInfo {
  const configVersion = Constants.expoConfig?.version || '0.1.5-alpha-e4f81';
  const configVersionCode = Constants.expoConfig?.android?.versionCode || 7;
  return {
