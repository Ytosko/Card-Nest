...
export async function runConfiguredExtraction(cardId: string, userId: string, imageUris: string[]) {
  ...
  try {
    ...
    const values = {
      ...
      primary_email: extracted.emails[0] || null,
      primary_phone: extracted.phones[0] || null,
    ...
    if (extracted.emails[0])
      relations.push(supabase.from('card_emails').insert({ user_id: userId, card_id: cardId, email: extracted.emails[0], is_primary: true }));
    if (extracted.phones[0])
      relations.push(supabase.from('card_phone_numbers').insert({ user_id: userId, card_id: cardId, phone_number: extracted.phones[0], is_primary: true }));
    if (extracted.websites[0])
      relations.push(supabase.from('card_websites').insert({ user_id: userId, card_id: cardId, url: extracted.websites[0], is_primary: true }));
